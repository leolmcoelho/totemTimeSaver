...
class Amil(Interation):
    ...
    def login(self, user, password):
        try:
            ...
            self.click_js(
                '/html/body/as-main-app/as-login-container/div[1]/div/as-login/div[2]/form/fieldset/button')
            print('clickou')
    ...
    def skip_login(self):
        try:
            self.click("finalizar-walktour", method='id', time=5)
        except:
    ...
    def insert_CPF(self, value='111'):
        ...
        self.driver.execute_script(
            'document.querySelector("#undefined > as-input-float-label > div.input-float-label > button").disabled = false;')
        # self.driver.execute_script('document.querySelector("#undefined > as-input-float-label > div.input-float-label > button").click();')
        self.click('//*[@id="undefined"]/as-input-float-label/div[1]/button')
        return True
    ...
    def set_atendimento(self, value):
        ...
        el = self.get_attribute(xpath)

        print(el)

        # self.locacated('//*[@id="results"]', 40)
        # print('ffoi')
        # self.driver.execute_script(js)
        # self.click_js(xpath2)
        #
    ...
    def insert_data(self):
        ...
        self.write('//*[@id="data-pedido-medico"]', date)
        return True

    def verify_result(self, html):
        try:
            while "Buscando" in self.element(html).get_attribute('innerHTML'):
                time.sleep(0.5)
        except:
            pass
    ...
    def set_solicitante(self, name):
        ...
        self.write(input_name, name)
        result = '//*[@id="results"]/li[1]'

        time.sleep(1)
        self.verify_result(result)

        self.element(input_name).send_keys(Keys.DOWN)
        self.element(input_name).send_keys(Keys.ENTER)
    ...
    def inserir_solicitante(self, name, CBO):
        ...
        for _ in range(5):
            el = self.get_attribute(input_name)
            if el == name:

                self.element(input_name).clear()
                self.set_solicitante(name)
                time.sleep(1.5)
            else:
                print(el)
        ...
        for _ in range(5):
            ...
            el = self.get_attribute(input_CBO)
            if el == CBO:

                self.element(input_CBO).clear()
                self.set_CBO(CBO)
                time.sleep(1.5)
            else:
                print(el)
    ...
    def inserir_servico(self, service="10101012"):
        self.write(
            '//*[@id="inclusao-consulta-pedido"]/section/as-tipo-pedido-sadt/div[5]/as-procedimento-servico/div/ul/li[1]/as-procedimento-autocomplete/div/div/input',
            service)
        self.click_js(
            '//*[@id="inclusao-consulta-pedido"]/section/as-tipo-pedido-sadt/div[5]/as-procedimento-servico/div/div/button')
        return True
    ...
    def click_autorization_previa(self):
        try:
            ...
            # self.get('https://credenciado.amil.com.br/pedidos-autorizacao')
            time.sleep(1)
            self.click_js('//*[@id="menu-usuario"]/as-item-menu[3]/li/a')
    ...
    def click_incluir(self):
        self.click_js('//*[@id="inclusao-consulta-pedido"]/section/div[4]/button[2]')
        return True

    def inserir_token(self, token):
        self.write('//*[@id="chave"]', token, time=40)
        self.click_js('//*[@id="tour3-confirma"]/button')
        logging.info("click no butao do token")
        return True

    def verify_token(self):
        response = self.element('//*[@id="detalhes-autorizacao"]/menu-pedido/as-message/div/p').text
    ...
    def get_senha(self):
        ...
        html = self.element(path, time=30).text.split('\n')
        if len(html) > 2:
            return html[1]
